...
function normalizeDistanceUnit(raw) {
    ...
    if (lower.startsWith('km') || lower.startsWith('kilometer'))
        return 'km';
    return 'miles';
}
// ─── Duration extraction ───
...
function extractDuration(message) {
    for (const pattern of DURATION_PATTERNS) {
        ...
        if (match) {
            if (/\bhalf\s+(?:an?\s+)?hour\b/i.test(match[0]))
                return 30;
            if (/\b(?:an?|one)\s+hour\b/i.test(match[0]))
                return 60;
...
// ─── Date extraction ───
function extractDate(message, timezone) {
    if (/\byesterday\b/i.test(message))
        return getYesterdayDate(timezone);
    // Default: today
    return getTodayDate(timezone);
}
// ─── Build description ───
function buildDescription(type, distance, distanceUnit, duration, people) {
    const parts = [type];
    if (distance != null) {
...
function applyDefaults(activity, defaults, userMentionedPeople) {
    if (!defaults)
        return activity;
...
export function extractActivityFromMessage(message, timezone, log, defaults) {
