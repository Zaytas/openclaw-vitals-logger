...
export function buildLoggedContext(activity) {
    ...
    return (`[Vitals Logger] Activity logged: ${summary}. Acknowledge briefly.`);
}
export function buildDuplicateContext(newActivity, existing) {
    ...
    return (`[Vitals Logger] Possible duplicate detected. New: "${newSummary}" matches existing: "${existingSummary}". ` +
        `Activity was NOT logged. Ask the user if this is a separate activity they want logged.`);
}
export function buildAmbiguousContext(missing) {
    return (`[Vitals Logger] Activity mentioned but couldn't auto-extract: missing ${missing.join(', ')}. ` +
