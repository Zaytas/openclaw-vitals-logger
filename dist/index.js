...
function isSubagentSession(ctx) {
    ...
    if (ctx.sessionKey && ctx.sessionKey.includes(':subagent:'))
        return true;
    if (ctx.session?.key && ctx.session.key.includes(':subagent:'))
        return true;
    if (ctx.parentSessionKey)
        return true;
    if (ctx.session?.parentKey)
        return true;
    return false;
}
    ...
    api.on('before_prompt_build', async (_event, ctx) => {
        try {
            ...
            if (!messages || messages.length === 0) {
                if (config.debug.logSkips)
                    log.info('Skip: no messages');
                return undefined;
            }
            const userMessage = extractLastUserMessage(messages);
            if (!userMessage) {
                if (config.debug.logSkips)
                    log.info('Skip: no user message found');
                return undefined;
            }
